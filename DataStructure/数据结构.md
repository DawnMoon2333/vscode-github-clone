...
node* CreateSqList(int nodesize){
...
status GetElemSqList(node* head, int i, ElemType* e){
    if (i < 1) return error;
    int index = 1;
    node* p = head;
